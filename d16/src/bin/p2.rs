        ...
        .filter(|(_i, properties)| {
            target_properties
                ...
                .all(|(target_property, target_value)| {
                    ...
                    if property_value_option.is_none() {
                        return true;
                    }
                    let property_value = property_value_option.unwrap();
                    match target_property.as_str() {
                        "cats" | "trees" => property_value > target_value,
                        "pomeranians" | "goldfish" => property_value < target_value,
                        _ => property_value == target_value,
                    }
                })
        })
        .map(|(i, _p)| i + 1)
